...
class PersonManager(models.Manager):
    ...
    def count_person_per_eye_color(self):
        return self.values(color=models.F('eye_color__color_name')).annotate(quantity=models.Count('eye_color'))
        ...
        with open(csv_file, mode='r') as csv_file:
            ...
            for i, row in enumerate(csv_reader):
                ...
                try:
                    ...
                    raise ValueError(
                        f"A cor do olho {row['eye_color']} não existe (linha {i + 1}). Valores validos: {valores_validos}")
...
class Person(models.Model):
    ...
    eye_color = models.ForeignKey(EyeColor, on_delete=models.CASCADE, null=True)
